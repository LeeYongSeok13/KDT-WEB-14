...
const practiceDetail = multer({
  storage: multer.diskStorage({
    ...
    filename(req, file, done) {
      ...
      done(null, path.basename(req.body.userId + ext));
    },
...
/* 
{
  ...
  originalname: 'Card1.jpg', // 원본 파일명
  encoding: '7bit', // 파일 인코딩 타입
  mimetype: 'image/jpeg', // 파일 타입
  destination: 'uploads/', // 파일 경로
  filename: 'bf6a1aab991c0a1cd55e71950328a3f3', // destination에 저장된 파일명
  path: 'uploads\\bf6a1aab991c0a1cd55e71950328a3f3', // 업로드된 파일 전체 경로
  size: 252802 // 파일 크기
}
[Object: null prototype] { title: '카드1' }
*/
...
app.post(
  ...
  (req, res) => {
    console.log(req.files); // {userfile1: [{파일 정보}], userfile2: [{...}]}
    console.log(req.body);
...
app.post(
  "/upload/fileUpload",
  practiceDetail.single("userfile"),
  (req, res) => {
    console.log(req.file);
    console.log(req.body);
    res.render("practice", {
